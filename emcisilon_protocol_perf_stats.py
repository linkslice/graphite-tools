...
def fetchOID(host, community, graphiteroot, emcProtocol, verbose):
    ...
    emcProtocolList = '.1.3.6.1.4.1.12124.2.2.10.1.1'
    Var = netsnmp.Varbind(emcProtocolList)
...
def main():
    ...
    parser.add_option('-b', '--emcProtocol', dest='emcProtocol',
        action='append',
        help='list of emcProtocols to fetch, if not specified will fetch entire list')
    ...
    #party time
    package, currentTime, result = fetchOID(options.host, options.community, options.graphiteroot, options.emcProtocol, options.verbose)
