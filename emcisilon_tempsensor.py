...
def fetchOID(host, community, graphiteroot, emcTempSensor, verbose):
    ...
    emcTempSensorList = '.1.3.6.1.4.1.12124.2.54.1.2'
    Var = netsnmp.Varbind(emcTempSensorList)
    ...
    for vs in Vars:
        ...
        if emcTempSensor:
            if vs.val not in emcTempSensor:
                continue
...
def main():
    ...
    parser.add_option('-b', '--emcTempSensor', dest='emcTempSensor',
        action='append',
        help='list of emcTempSensors to fetch, if not specified will fetch entire list')
    ...
    #party time
    package, currentTime, result = fetchOID(options.host, options.community, options.graphiteroot, options.emcTempSensor, options.verbose)
