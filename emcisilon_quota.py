...
def fetchOID(host, community, graphiteroot, emcQuota, verbose):
    ...
    emcQuotaList = '.1.3.6.1.4.1.12124.1.12.1.1.5'
    Var = netsnmp.Varbind(emcQuotaList)
...
def main():
    ...
    parser.add_option('-b', '--emcQuota', dest='emcQuota',
        action='append',
        help='list of emcQuotas to fetch, if not specified will fetch entire list')
    ...
    #party time
    package, currentTime, result = fetchOID(options.host, options.community, options.graphiteroot, options.emcQuota, options.verbose)
