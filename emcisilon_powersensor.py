...
def fetchOID(host, community, graphiteroot, emcPowerSensor, verbose):
    ...
    emcPowerSensorList = '.1.3.6.1.4.1.12124.2.55.1.2'
    Var = netsnmp.Varbind(emcPowerSensorList)
    ...
    for vs in Vars:
        ...
        if emcPowerSensor:
            if vs.val not in emcPowerSensor:
                continue
...
def main():
    ...
    parser.add_option('-b', '--emcPowerSensor', dest='emcPowerSensor',
        action='append',
        help='list of emcPowerSensors to fetch, if not specified will fetch entire list')
    ...
    #party time
    package, currentTime, result = fetchOID(options.host, options.community, options.graphiteroot, options.emcPowerSensor, options.verbose)
