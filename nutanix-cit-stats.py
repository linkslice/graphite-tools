...
def fetchOID(host, username, seclevel,
             ...
             graphiteroot, cit, verbose):
    if verbose:
        print('connecting to host: %s using username: %s' % ( host, username ), file=sys.stderr)

    statsTable = {
        'citTotalCapacity': '1.3.6.1.4.1.41263.8.1.4',
        'citUsedCapacity': '1.3.6.1.4.1.41263.8.1.5',
        'citIOPerSecond': '1.3.6.1.4.1.41263.8.1.6',
        'citAvgLatencyUsecs': '1.3.6.1.4.1.41263.8.1.7'
        }
    ...
    nutanixContainerList = '.1.3.6.1.4.1.41263.8.1.1'

    Var = netsnmp.Varbind(nutanixContainerList)
    Vars = netsnmp.VarList(Var)
    snmp.walk(Vars)
    currentTime = time.time()
    for container in Vars:
        ...
        if cit:
            if container.val not in cit:
                continue
        for type in statsTable:
            try:
                # fetch 'citContainerId' which I understand is the scsi id
                citContainerIdOid = '1.3.6.1.4.1.41263.8.1.3' + '.' + container.iid
                head, tail = citContainerIdOid.rsplit('.', 1)
                vars = netsnmp.VarList(netsnmp.Varbind(head, tail))
                result = [x[0] for x in snmp.get(vars)]
                citContainerId = x
                # now fetch stats
                oid = statsTable[type] + "." + container.iid
                head, tail = oid.rsplit('.', 1)
                vars = netsnmp.VarList(netsnmp.Varbind(head, tail))
                result = [x[0] for x in snmp.get(vars)]
                result = float(x)
                if verbose:
                    print('%s %s = %s' % (container.val, type, result), file=sys.stderr)
                #currentTime = time.time()
                datapoint = '%s.%s.%s' % (graphiteroot, citContainerId, type)
...
def main():
    ...
    parser.add_option('-s', '--cit', dest='cit',
        action='append',
        help='Container Stat table')
    ...
    package, currentTime, result = fetchOID(options.host, options.username, options.seclevel, 
                                            ...
                                            options.graphiteroot, options.cit, options.verbose)
